                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{ route('operator.tablereport') }}" method="GET" class="mb-3">
                                            ...
                                            <tr style="background-color: {{$rowIndex < 3 ? '#6add6a' : ($rowIndex < 6 ? '#f1dc48' : ($rowIndex < 9 ? '#f19648' : '#f16363')) }}">
                                                ...
                                                @foreach (Carbon\CarbonPeriod::create($from_date, $to_date) as $date)
                                                    <td style="border-width: 1px 0; padding: 10px 0; text-align: center; align-items: center;">
                                                        <a href="/operator/productoper?operator={{ $id }}&from_date={{ $date }}&to_date={{ $date }}&date={{ $date->format('Y-m-d') }}" style="text-decoration: none; color: inherit;">
                                                            {{ isset($averages[$id][$date->format('Y-m-d')]) ? number_format($averages[$id][$date->format('Y-m-d')], 1) : '-' }}
                                                        </a>
                                                    </td> <!-- Format to 1 decimal places -->
                                                    @endforeach
                                                    <td>
                                                        <a href="/operator/productoper?operator={{ $id }}&from_date={{ $from_date }}&to_date={{ $to_date }}&date={{ $date->format('Y-m-d') }}" style="text-decoration: none; color: inherit;">
